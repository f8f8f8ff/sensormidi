...
class sensor {
   public:
    ...
    sensor(uint8_t pin_trig = 0, uint8_t pin_echo = 0,
...
class sensor_manager {
   public:
    ...
    void read();
    int value(int n);
    sensor_values* update_all_values();
    void echo_check();
    void print();
    sensor_manager() {};
};
...
void sensor_manager::add_sensor(sensor s) {
...
void _echo_check() { sensors.echo_check(); }

void sensor_manager::read() {
    bool DEBUG = false;
...
void sensor_manager::echo_check() {
...
int sensor_manager::value(int n) { return sensors[n].filter.read(); }

sensor_values* sensor_manager::update_all_values() {
    values.len = n_sensors;
    for (int i = 0; i < n_sensors; i++) {
        values.s[i] = value(i);
    }
