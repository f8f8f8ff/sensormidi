...
int cmd_arg_int_read(int *args, int n_args, char* s) {
    char* token;
    token = strtok(s, " ");
...
void cmd_run(cmd_t c, char* input, size_t input_l) {
    switch (c.type) {
        ...
        case cmd_arg_t::INTARG:
            int arg;
            if (cmd_arg_int_read(&arg, 1, input) != 1) break;
            ((cmd_fn_intarg)c.cmd)(arg);
            break;
        case cmd_arg_t::INTARG_3:
            int args[3];
            int l = cmd_arg_int_read(args, 3, input);
            if (l != 3) break;
